...
ℹ️ This music tracker produces Sonant formatted JSON. Sonant based synth is designed to be small, for size constrained games (player+song = 2k).  
...
- Normally, the song is output as JSON to use with a `sonant player`, but it can also be exported to `.wav` by long-pressing on the **waveform** visualizer. It will export whatever is selected: tracks/phrases or whole song.
