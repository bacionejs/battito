...
- The **textarea** can be edited. Changes are **live** just like the other components. This is helpful, especially when adjusting envelope values, as they have a large range.
...
-  To do **live jamming**, **double-click the waveform visualizer** to open the **improv four-octave keyboard** in a separate browser tab. You can select any of your **8 instruments** by **long-pressing a piano key (1–8)**. The improv tab receives **real-time instrument setting updates** from the main app (sliders). Note that there is **currently no support** to route your improv performance back into the sequencer.
