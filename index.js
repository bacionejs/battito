...
function initSequencer() {
  ...
  element("div", sequencer, "cell col-header");// Top-left corner cell
  ...
  song.activeTracks.forEach((isActive, trackIndex) => {
    ...
    if (currSequence !== null) {
      ...
      pattern.forEach((note, row) => {
        if (!note) return;
        let col = note - 123; // Note value to column index
...
function updateSequencer() {
  ...
  cells.forEach(cell => {
    ...
    if (row === undefined && col === undefined) return; //corner
    if (row === undefined) { 
