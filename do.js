...
function handleDownloadRequest() {
    ...
    if (!isAnySequenceActive) {
        alert("Please select at least one sequence (row) to download.");
        canvas.style.borderColor = "silver";
        return;
    }
    const isAnyTrackActive = song.activeTracks.some(x => x);
    let originalActiveTracks = null;
    if (!isAnyTrackActive) {
        originalActiveTracks = [...song.activeTracks];
        song.activeTracks.fill(1);
    }
    const songToRender = getActiveSongDataForPlayback();
    if (originalActiveTracks) {
        song.activeTracks = originalActiveTracks;
    }
    if (!songToRender[1] || songToRender[1].length === 0) {
        alert("Nothing to render. Please select tracks or ensure selected sequences have patterns.");
        canvas.style.borderColor = "silver";
        return;
    }
    PLAYER(audioContext, synth => {
        try {
            const buffer = synth.song(songToRender);
            if (!buffer || buffer.length === 0) { throw new Error("Rendered buffer is empty."); }
            const wavBlob = bufferToWav(buffer);
            const timestamp = new Date().toISOString().replace(/[-:.]/g, "").slice(0, 15);
            const filename = `battito_${timestamp}.wav`;
            ...
            alert("Could not generate audio for download. The selected range might be empty.");
        } finally {
            canvas.style.borderColor = "silver";
        }
    });
...
const handleCanvasPressStart=e=>{e.preventDefault();longPressTimer=setTimeout(handleDownloadRequest,longPressDuration);};
const handleCanvasPressEnd=e=>{e.preventDefault();clearTimeout(longPressTimer);};
canvas.addEventListener("mousedown",handleCanvasPressStart);
canvas.addEventListener("mouseup",handleCanvasPressEnd);
canvas.addEventListener("mouseleave",handleCanvasPressEnd);
canvas.addEventListener("touchstart",handleCanvasPressStart,{passive:false});
canvas.addEventListener("touchend",handleCanvasPressEnd);
canvas.addEventListener("touchcancel",handleCanvasPressEnd);
}
